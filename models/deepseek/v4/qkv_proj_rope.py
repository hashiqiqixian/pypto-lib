...
def qkv_proj_rope(
    ...
    for tg_idx in pl.spmd((t_dim + T_TILE - 1) // T_TILE, name_hint="qr_rms_norm_quant", allow_early_resolve=True):
        ...
        for qa in pl.pipeline(0, Q_LORA, QUANT_TILE, stage=2):
            ...
            pl.store(pl.set_validshape(qr_q_i8, valid_rows, QUANT_TILE), [tg, qa], qr_view)
    ...
    for tg_idx in pl.spmd((t_dim + KV_RMS_T_TILE - 1) // KV_RMS_T_TILE, name_hint="kv_rms_norm_rope"):
        ...
        kv_cos_rows = pl.slice(rope_cos_view, [KV_RMS_T_TILE, ROPE_DIM], [tg, 0], valid_shape=[valid_rows, ROPE_DIM])
        kv_sin_rows = pl.slice(rope_sin_view, [KV_RMS_T_TILE, ROPE_DIM], [tg, 0], valid_shape=[valid_rows, ROPE_DIM])
        kv_cos_il = pl.gather(pl.cast(kv_cos_rows, target_type=pl.FP32), dim=-1, index=kv_dup_idx)
        kv_sin_il = pl.gather(pl.cast(kv_sin_rows, target_type=pl.FP32), dim=-1, index=kv_dup_idx)
