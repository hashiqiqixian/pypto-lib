...
# -----------------------------------------------------------------------------------------------------------
"""Request-owned ring storage shared by C2A prefill and decode."""
...
from models.deepseek_v4_1_flash.config import FLASH, HEAD_DIM, MAX_BATCH_PER_DP


@dataclass(frozen=True)
class CompressorStateSnapshot:
    """Detached per-source ring blocks at a completed prefix boundary."""

    prefix_length: int
    rings: Mapping[int, torch.Tensor]


class CompressorStateCache:
    """Own stable blocks and per-source FP32 ring buffers on one TP rank.
    ...
    explicitly), and snapshot only after all source layers finish. Ring storage does not
    implement speculative acceptance, rejection or rollback by itself.
    """

    def __init__(
        self, *, num_blocks: int = MAX_BATCH_PER_DP, capacity: int = 2,
        device: torch.device | str = "cpu",
    ) -> None:
        if type(num_blocks) is not int or num_blocks <= 0 or type(capacity) is not int or capacity <= 0:
            raise ValueError("state block count and ring capacity must be positive integers")
        self.capacity = capacity
        self._buffers = {
            source: torch.zeros((num_blocks, capacity, 2 * HEAD_DIM), dtype=torch.float32, device=device)
    ...
    def allocate(
        ...
        *,
        ...
        if snapshot is not None:
            ...
            if snapshot.prefix_length != prefix_length or set(snapshot.rings) != set(self._buffers):
                raise ValueError("snapshot must match the prefix length and all compressor sources")
            for pending in snapshot.rings.values():
                if pending.shape != (self.capacity, 2 * HEAD_DIM) or pending.dtype != torch.float32:
                    raise ValueError("snapshot rings must be FP32 [capacity, 2 * HEAD_DIM]")
        slot = self._free[-1]
        ...
        for source, buffer in self._buffers.items():
            if snapshot is None:
                ...
                buffer[slot].copy_(snapshot.rings[source])
        self._free.pop()
        self._owners[request_key] = slot
        return slot

    def block_tables(self, request_keys: Sequence[Hashable]) -> Mapping[int, torch.Tensor]:
        """Build batch-ordered state block tables for build_forward_metadata.
        ...
        return {
            source: torch.tensor(rows, dtype=torch.int32, device=buffer.device).reshape(-1, 1)
            for source, buffer in self._buffers.items()
        }
    ...
    def release(self, request_key: Hashable) -> None:
        """Clear the entire ring block before making a completed request's slot reusable."""
        slot = self._owners[request_key]
