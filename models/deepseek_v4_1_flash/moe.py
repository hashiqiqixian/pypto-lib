...
normalize = make_norm(C.D)
shared_up = make_projection(C.D, C.MOE_INTER)
shared_down = make_projection(C.MOE_INTER, C.D)
...
def make_routed_projection(width, output_width):
    ...
    @pl.jit.inline
    def project(
        x: pl.Tensor[[C.T_DYN, width], pl.BF16],
        weight: pl.Tensor[[output_width, width], pl.FP4],
        scale: pl.Tensor[[output_width, width // C.MX_GROUP], pl.FP8E8M0],
...
routed_up = make_routed_projection(C.D, C.MOE_INTER)
routed_down = make_routed_projection(C.MOE_INTER, C.D)


@pl.jit.inline
def swiglu(
    gate_value: pl.Tensor[[C.T_DYN, C.MOE_INTER], pl.BF16],
    up_value: pl.Tensor[[C.T_DYN, C.MOE_INTER], pl.BF16],
    weights: pl.Tensor[[C.T_DYN, C.AUX_WIDTH], pl.FP32],
    hidden: pl.Tensor[[C.T_DYN, C.MOE_INTER], pl.BF16],
    num_tokens: pl.Scalar[pl.INT32],
):
    for token in pl.spmd(num_tokens, name_hint="moe_swiglu"):
        gate_row = pl.minimum(pl.cast(gate_value[token:token + 1, :], pl.FP32), FLASH.swiglu_limit)
        up_row = pl.minimum(pl.maximum(pl.cast(up_value[token:token + 1, :], pl.FP32), -FLASH.swiglu_limit), FLASH.swiglu_limit)
...
def route(
    ...
    indices: pl.Tensor[[C.T_DYN, C.TOPK], pl.INT32],
    weights: pl.Tensor[[C.T_DYN * C.TOPK, C.AUX_WIDTH], pl.FP32],
    routes: pl.Tensor[[C.T_DYN * C.TOPK, C.ROUTE_WIDTH], pl.INT32],
    num_tokens: pl.Scalar[pl.INT32],
):
    tokens = pl.tensor.dim(x, 0)
    scores = pl.create_tensor([tokens, C.N_EXPERTS], dtype=pl.FP32)
    for block in pl.spmd((num_tokens + 15) // 16 * (C.N_EXPERTS // 32), name_hint="moe_gate"):
        token = block // (C.N_EXPERTS // 32) * 16
        column = block % (C.N_EXPERTS // 32) * 32
        rows = pl.min(16, num_tokens - token)
        logits = pl.create_tensor([16, 32], dtype=pl.FP32)
        for kb in pl.range(C.D // K_TILE):
        ...
        logits = pl.mul(logits, 1.0 / FLASH.gate_temperature)
    ...
    for block in pl.spmd((num_tokens + 7) // 8, name_hint="moe_topk"):
        for token in pl.range(block * 8, pl.min((block + 1) * 8, num_tokens)):
            ...
            bias = pl.reshape(correction_bias[:], [1, C.N_EXPERTS])
            expert_ids = pl.arange(0, [1, C.N_EXPERTS], dtype=pl.UINT32)
            ...
            valid_scores = pl.fillpad(pl.set_validshape(selected_scores, 1, C.TOPK), pad_value=pl.PadValue.zero)
            denominator = pl.add(pl.row_sum(valid_scores), 1e-20)
            normalized = pl.mul(pl.row_expand_div(valid_scores, denominator), FLASH.routed_scaling_factor)
            for k in pl.range(C.TOPK):
                pl.write(indices, [token, k], pl.read(selected, [0, k]))
                route_id = token * C.TOPK + k
                weight_row = pl.full([1, C.AUX_WIDTH], dtype=pl.FP32, value=0.0)
                route_row = pl.full([1, C.ROUTE_WIDTH], dtype=pl.INT32, value=0)
...
def moe(
    ...
    route_rows = tokens * C.TOPK
    x_norm = pl.create_tensor([tokens, C.D], dtype=pl.BF16)
    x_quant = pl.create_tensor([tokens, C.D], dtype=pl.FP8E4M3FN)
    x_scale = pl.create_tensor([tokens, C.D // 32], dtype=pl.UINT8)
    indices = pl.create_tensor([tokens, C.TOPK], dtype=pl.INT32)
    weights = pl.create_tensor([route_rows, C.AUX_WIDTH], dtype=pl.FP32)
    routes = pl.create_tensor([route_rows, C.ROUTE_WIDTH], dtype=pl.INT32)
    ...
    with pl.at(level=pl.Level.CORE_GROUP, name_hint="moe_reuse_wait") as reuse_tid:
        if moe_epoch > 1:
            for peer in pl.range(C.EP_SIZE):
                if peer != ep_rank:
    ...
    counts = pl.create_tensor([C.EP_SIZE, C.N_LOCAL_EXPERTS], dtype=pl.INT32)
    with pl.spmd(C.EP_SIZE, name_hint="moe_dispatch", deps=[reuse_tid]) as dispatch_tid:
        destination = pl.tile.get_block_idx()
        cursor = pl.array.create(C.N_LOCAL_EXPERTS, pl.INT32)
        for expert in pl.range(C.N_LOCAL_EXPERTS):
            cursor[expert] = 0
        for token in pl.range(active):
            if pl.read(token_owners, [token]) == tp_rank:
                for k in pl.range(C.TOPK):
                    expert_id = pl.read(indices, [token, k])
                    if expert_id // C.N_LOCAL_EXPERTS == destination:
                        local_expert = expert_id % C.N_LOCAL_EXPERTS
                        row = local_expert * C.RECV_MAX + ep_rank * SOURCE_CAPACITY + cursor[local_expert]
                        cursor[local_expert] = cursor[local_expert] + 1
                        peer = group_base + destination
                        route_id = token * C.TOPK + k
                        pld.tensor.put(dst=recv_x, peer=peer, src=x_quant,
                                       dst_offsets=[row, 0], src_offsets=[token, 0], shape=[1, C.D])
                        pld.tensor.put(dst=recv_scale, peer=peer, src=x_scale,
                                       dst_offsets=[row, 0], src_offsets=[token, 0], shape=[1, C.D // 32])
                        pld.tensor.put(dst=recv_weights, peer=peer, src=weights,
                                       dst_offsets=[row, 0], src_offsets=[route_id, 0], shape=[1, C.AUX_WIDTH])
                        pld.tensor.put(dst=recv_routes, peer=peer, src=routes,
                                       dst_offsets=[row, 0], src_offsets=[route_id, 0], shape=[1, C.ROUTE_WIDTH])
        count_row = pl.full([1, C.N_LOCAL_EXPERTS], dtype=pl.INT32, value=0)
        for expert in pl.range(C.N_LOCAL_EXPERTS):
        ...
        pld.tensor.put(dst=recv_meta, peer=group_base + destination, src=counts,
                       dst_offsets=[ep_rank, 0], src_offsets=[destination, 0], shape=[1, C.N_LOCAL_EXPERTS])
    ...
    recv_counts = pl.create_tensor([C.EP_SIZE, C.N_LOCAL_EXPERTS], dtype=pl.INT32)
    ...
    expert_tids = pl.array.create(C.N_LOCAL_EXPERTS, pl.TASK_ID)
    for expert in pl.range(C.N_LOCAL_EXPERTS):
        source_tids = pl.array.create(C.EP_SIZE, pl.TASK_ID)
        for source in pl.range(C.EP_SIZE):
            ...
            for chunk in pl.range((source_rows + EXPERT_TILE - 1) // EXPERT_TILE):
                ...
                row_base = expert * C.RECV_MAX + source * SOURCE_CAPACITY + chunk * EXPERT_TILE
                with pl.scope():
                    expert_x = pl.create_tensor([EXPERT_TILE, C.D], dtype=pl.BF16)
                    expert_weights = pl.create_tensor([EXPERT_TILE, C.AUX_WIDTH], dtype=pl.FP32)
                    gate_value = pl.create_tensor([EXPERT_TILE, C.MOE_INTER], dtype=pl.BF16)
                    up_value = pl.create_tensor([EXPERT_TILE, C.MOE_INTER], dtype=pl.BF16)
                    hidden = pl.create_tensor([EXPERT_TILE, C.MOE_INTER], dtype=pl.BF16)
                    result = pl.create_tensor([EXPERT_TILE, C.D], dtype=pl.BF16)
                    send_result = pl.create_tensor([EXPERT_TILE, C.D], dtype=pl.FP32)
                    with pl.spmd(rows, name_hint="moe_received_input", deps=[wait_tid]) as _received_tid:
                        row = pl.tile.get_block_idx()
                        value = pl.reshape(pl.cast(recv_x[row_base + row:row_base + row + 1, :], pl.FP32), [C.D // 32, 32])
                        codes = pl.ands(pl.cast(pl.reinterpret_view(recv_scale[row_base + row:row_base + row + 1, :], pl.INT8), pl.INT32), 255)
                        factor = pl.reshape(pl.reinterpret_view(pl.maximum(pl.shls(codes, 23), 4194304), pl.FP32), [C.D // 32, 1])
                        expert_x[row:row + 1, :] = pl.reshape(pl.cast(pl.row_expand_mul(value, factor), pl.BF16, mode="rint"), [1, C.D])
                    ...
                    with pl.at(level=pl.Level.CORE_GROUP, name_hint="moe_expert_scatter") as scatter_tid:
                        source_base = source // C.TP_SIZE * C.TP_SIZE
                        for row in pl.range(rows):
                            route_id = pl.read(recv_routes, [row_base + row, 0])
                            for replica in pl.range(C.TP_SIZE):
    ...
    combined_tids = pl.array.create((C.PREFILL_MAX_TOKENS + EXPERT_TILE - 1) // EXPERT_TILE, pl.TASK_ID)
    for chunk in pl.range((active + EXPERT_TILE - 1) // EXPERT_TILE):
        start = chunk * EXPERT_TILE
        rows = pl.min(EXPERT_TILE, active - start)
        with pl.scope():
            shared_input = pl.slice(x_norm, [EXPERT_TILE, C.D], [start, 0], valid_shape=[rows, C.D])
            gate_value = pl.create_tensor([EXPERT_TILE, C.MOE_INTER], dtype=pl.BF16)
            up_value = pl.create_tensor([EXPERT_TILE, C.MOE_INTER], dtype=pl.BF16)
            hidden = pl.create_tensor([EXPERT_TILE, C.MOE_INTER], dtype=pl.BF16)
            shared_result = pl.create_tensor([EXPERT_TILE, C.D], dtype=pl.BF16)
            shared_weights = pl.create_tensor([EXPERT_TILE, C.AUX_WIDTH], dtype=pl.FP32)
            with pl.at(level=pl.Level.CORE_GROUP, name_hint="moe_shared_weights"):
                shared_weights[:, :] = pl.full([EXPERT_TILE, C.AUX_WIDTH], dtype=pl.FP32, value=1.0)
            ...
            with pl.spmd(rows, name_hint="moe_combine", deps=[combine_tid]) as combined_tid:
                row = pl.tile.get_block_idx()
                value = pl.full([1, C.D], dtype=pl.FP32, value=0.0)
                for k in pl.range(C.TOPK):
                    route_id = (start + row) * C.TOPK + k
...
def make_program(epochs=2):
    ...
    def moe_rank(
        ...
        gate_weight: pl.Tensor[[C.N_EXPERTS, C.D], pl.FP32],
        correction_bias: pl.Tensor[[C.N_EXPERTS], pl.FP32],
        routed_w1: pl.Tensor[[C.N_LOCAL_EXPERTS, C.MOE_INTER, C.D // 2], pl.UINT8],
        routed_w1_scale: pl.Tensor[[C.N_LOCAL_EXPERTS, C.MOE_INTER, C.D // 32], pl.FP8E8M0],
        routed_w2: pl.Tensor[[C.N_LOCAL_EXPERTS, C.D, C.MOE_INTER // 2], pl.UINT8],
        routed_w2_scale: pl.Tensor[[C.N_LOCAL_EXPERTS, C.D, C.MOE_INTER // 32], pl.FP8E8M0],
        routed_w3: pl.Tensor[[C.N_LOCAL_EXPERTS, C.MOE_INTER, C.D // 2], pl.UINT8],
        routed_w3_scale: pl.Tensor[[C.N_LOCAL_EXPERTS, C.MOE_INTER, C.D // 32], pl.FP8E8M0],
        shared_w1: pl.Tensor[[C.D, C.MOE_INTER], pl.FP8E4M3FN],
        shared_w1_scale: pl.Tensor[[C.D // 32, C.MOE_INTER], pl.FP8E8M0, pl.MX_B_NN],
        shared_w2: pl.Tensor[[C.MOE_INTER, C.D], pl.FP8E4M3FN],
        shared_w2_scale: pl.Tensor[[C.MOE_INTER // 32, C.D], pl.FP8E8M0, pl.MX_B_NN],
        shared_w3: pl.Tensor[[C.D, C.MOE_INTER], pl.FP8E4M3FN],
        shared_w3_scale: pl.Tensor[[C.D // 32, C.MOE_INTER], pl.FP8E8M0, pl.MX_B_NN],
        token_owners: pl.Tensor[[C.T_DYN], pl.INT32],
        recv_meta: pld.DistributedTensor[[C.EP_SIZE, C.N_LOCAL_EXPERTS], pl.INT32],
        recv_x: pld.DistributedTensor[[C.N_LOCAL_EXPERTS * C.RECV_MAX, C.D], pl.FP8E4M3FN],
        recv_scale: pld.DistributedTensor[[C.N_LOCAL_EXPERTS * C.RECV_MAX, C.D // 32], pl.UINT8],
        recv_weights: pld.DistributedTensor[[C.N_LOCAL_EXPERTS * C.RECV_MAX, C.AUX_WIDTH], pl.FP32],
        recv_routes: pld.DistributedTensor[[C.N_LOCAL_EXPERTS * C.RECV_MAX, C.ROUTE_WIDTH], pl.INT32],
        arrived: pld.DistributedTensor[[C.EP_SIZE, 1], pl.INT32],
        data_arrived: pld.DistributedTensor[[C.EP_SIZE, 1], pl.INT32],
    ...
    @pl.jit.host
    def moe_group(
        x: pl.Tensor[[C.EP_SIZE, C.T_DYN, C.D], pl.BF16],
        norm_weight: pl.Tensor[[C.EP_SIZE, C.D], pl.BF16],
        gate_weight: pl.Tensor[[C.EP_SIZE, C.N_EXPERTS, C.D], pl.FP32],
        correction_bias: pl.Tensor[[C.EP_SIZE, C.N_EXPERTS], pl.FP32],
        routed_w1: pl.Tensor[[C.EP_SIZE, C.N_LOCAL_EXPERTS, C.MOE_INTER, C.D // 2], pl.UINT8],
        routed_w1_scale: pl.Tensor[[C.EP_SIZE, C.N_LOCAL_EXPERTS, C.MOE_INTER, C.D // 32], pl.FP8E8M0],
        routed_w2: pl.Tensor[[C.EP_SIZE, C.N_LOCAL_EXPERTS, C.D, C.MOE_INTER // 2], pl.UINT8],
        routed_w2_scale: pl.Tensor[[C.EP_SIZE, C.N_LOCAL_EXPERTS, C.D, C.MOE_INTER // 32], pl.FP8E8M0],
        routed_w3: pl.Tensor[[C.EP_SIZE, C.N_LOCAL_EXPERTS, C.MOE_INTER, C.D // 2], pl.UINT8],
        routed_w3_scale: pl.Tensor[[C.EP_SIZE, C.N_LOCAL_EXPERTS, C.MOE_INTER, C.D // 32], pl.FP8E8M0],
        shared_w1: pl.Tensor[[C.EP_SIZE, C.D, C.MOE_INTER], pl.FP8E4M3FN],
        shared_w1_scale: pl.Tensor[[C.EP_SIZE, C.D // 32, C.MOE_INTER], pl.FP8E8M0],
        shared_w2: pl.Tensor[[C.EP_SIZE, C.MOE_INTER, C.D], pl.FP8E4M3FN],
        shared_w2_scale: pl.Tensor[[C.EP_SIZE, C.MOE_INTER // 32, C.D], pl.FP8E8M0],
        shared_w3: pl.Tensor[[C.EP_SIZE, C.D, C.MOE_INTER], pl.FP8E4M3FN],
        shared_w3_scale: pl.Tensor[[C.EP_SIZE, C.D // 32, C.MOE_INTER], pl.FP8E8M0],
        token_owners: pl.Tensor[[C.EP_SIZE, C.T_DYN], pl.INT32],
        output: pl.Out[pl.Tensor[[C.EP_SIZE, C.T_DYN, C.D], pl.BF16]],
        num_tokens: pl.Scalar[pl.INT32],
    ):
        x.bind_dynamic(1, C.T_DYN)
        tokens = pl.tensor.dim(x, 1)
        route_rows = tokens * C.TOPK
        meta_buf = pld.alloc_window_buffer([C.EP_SIZE, C.N_LOCAL_EXPERTS], dtype=pl.INT32)
        x_buf = pld.alloc_window_buffer([C.N_LOCAL_EXPERTS * C.RECV_MAX, C.D], dtype=pl.FP8E4M3FN)
        scale_buf = pld.alloc_window_buffer([C.N_LOCAL_EXPERTS * C.RECV_MAX, C.D // 32], dtype=pl.UINT8)
        weights_buf = pld.alloc_window_buffer([C.N_LOCAL_EXPERTS * C.RECV_MAX, C.AUX_WIDTH], dtype=pl.FP32)
        routes_buf = pld.alloc_window_buffer([C.N_LOCAL_EXPERTS * C.RECV_MAX, C.ROUTE_WIDTH], dtype=pl.INT32)
        arrived_buf = pld.alloc_window_buffer([C.EP_SIZE, 1], dtype=pl.INT32)
        data_buf = pld.alloc_window_buffer([C.EP_SIZE, 1], dtype=pl.INT32)
        output_buf = pld.alloc_window_buffer([route_rows, C.D], dtype=pl.FP32)
        combine_buf = pld.alloc_window_buffer([C.EP_SIZE, 1], dtype=pl.INT32)
        for rank in pl.range(pld.world_size()):
            recv_meta = pld.window(meta_buf, [C.EP_SIZE, C.N_LOCAL_EXPERTS], dtype=pl.INT32)
            recv_x = pld.window(x_buf, [C.N_LOCAL_EXPERTS * C.RECV_MAX, C.D], dtype=pl.FP8E4M3FN)
            recv_scale = pld.window(scale_buf, [C.N_LOCAL_EXPERTS * C.RECV_MAX, C.D // 32], dtype=pl.UINT8)
            recv_weights = pld.window(weights_buf, [C.N_LOCAL_EXPERTS * C.RECV_MAX, C.AUX_WIDTH], dtype=pl.FP32)
            recv_routes = pld.window(routes_buf, [C.N_LOCAL_EXPERTS * C.RECV_MAX, C.ROUTE_WIDTH], dtype=pl.INT32)
            arrived = pld.window(arrived_buf, [C.EP_SIZE, 1], dtype=pl.INT32)
            data_arrived = pld.window(data_buf, [C.EP_SIZE, 1], dtype=pl.INT32)
            routed_output = pld.window(output_buf, [route_rows, C.D], dtype=pl.FP32)
            combine_arrived = pld.window(combine_buf, [C.EP_SIZE, 1], dtype=pl.INT32)
            moe_rank(
