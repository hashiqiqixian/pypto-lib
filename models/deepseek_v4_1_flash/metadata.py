    ...
    *,
    ...
    for source in FLASH.kv_source_layer_ids:
        ...
        if ratio > 1:
            ...
            active_slots = slots[query_lens > 0]
            if bool(((active_slots < 0) | (active_slots >= MAX_BATCH_PER_DP)).any()):
                raise ValueError("active compressor state slot is outside the kernel state pool")
            if active_slots.unique().numel() != active_slots.numel():
                raise ValueError("active requests must own distinct compressor state slots")
