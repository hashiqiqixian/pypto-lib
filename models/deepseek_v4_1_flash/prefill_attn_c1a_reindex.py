...
# -----------------------------------------------------------------------------------------------------------
"""Packed-prefill C1A reindex attention leaf."""
...
def golden_prefill_attn_c1a_reindex(
    x: torch.Tensor,
    wq_a: torch.Tensor,
...
def make_prefill_attn_c1a_reindex(indexer):
    ...
    return prefill_attn_c1a_reindex_impl


prefill_attn_c1a_reindex = make_prefill_attn_c1a_reindex(paged_indexer)
prefill_attn_c1a_reindex_watch = make_prefill_attn_c1a_reindex(paged_indexer_direct)


@pl.jit
def prefill_attn_c1a_reindex_test(
    x: pl.Tensor[[C.T_DYN, C.D], pl.BF16],
    wq_a: pl.Tensor[[C.D, C.Q_LORA], pl.FP8E4M3FN],
    wq_a_scale: pl.Tensor[[C.D // 32, C.Q_LORA], pl.FP8E8M0, pl.MX_B_NN],
    q_norm_weight: pl.Tensor[[C.Q_LORA], pl.BF16],
    ...
    return prefill_attn_c1a_reindex(
...
@pl.jit.host
def l3_prefill_attn_c1a_reindex_test(
...
def golden_prefill_attn_c1a_reindex_case(tensors):
    apply_distributed_golden("reindex", golden_prefill_attn_c1a_reindex, tensors)


__all__ = ["golden_prefill_attn_c1a_reindex", "prefill_attn_c1a_reindex"]

# Keep the pre-split names available to the HC orchestration wrapper.
golden_prefill_c1a_reindex = golden_prefill_attn_c1a_reindex
prefill_c1a_reindex = prefill_attn_c1a_reindex
...
def validate(argv=None):
    ...
    result = run(
        fn=l3_prefill_attn_c1a_reindex_test,
        specs=build_tensor_specs(args.tokens, args.case),
        golden_fn=golden_prefill_attn_c1a_reindex_case,
