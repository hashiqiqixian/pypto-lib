...
def backbone_reference(
    query_hc: torch.Tensor,
    layer_functions: Sequence[Callable[[torch.Tensor, int], torch.Tensor]],
    head_function: Callable[[torch.Tensor], torch.Tensor],
    query_width: int,
) -> tuple[list[torch.Tensor], torch.Tensor]:
    """Run three synthetic draft layers and return their intermediates."""
    hidden = query_hc
    intermediates = []
    for layer_id, layer_function in enumerate(layer_functions):
        hidden = layer_function(hidden, layer_id)
        intermediates.append(hidden.clone())
    head_hidden = head_function(hidden[:, :query_width])
    return intermediates, head_hidden
