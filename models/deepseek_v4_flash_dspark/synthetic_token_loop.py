...
It is a validation driver, not a Serving implementation.  One logical request
is owned by the first rank in each TP group.  Decode has no inactive-query mask,
so peer ranks retain valid fixture-owned dummy requests; only the leader request
is handed to the drafter and verifier.  Target-model resident caches and the
TP-sharded output head are shared across stages, while drafter KV is retained
independently.  Host verification uses the same longest-prefix-plus-one rule
required by Serving.  The bounded fixture validates ABI, ownership, and
resident-state flow; its Prefill and one-bank Decode layer weights are
independently initialized, so it is not an end-to-end numerical golden for one
checkpoint.
"""
...
def _resident_names(specs, *, output_state_names=frozenset()):
    return {
        spec.name
        for spec in specs
        if (
            (getattr(spec, "is_resident", False) and not spec.is_output)
...
def _gather_prefill_target_hidden(hidden_local: torch.Tensor, tp_size: int = TP_SIZE) -> torch.Tensor:
    """Pack each TP group's rank-major prompt rows for its stable owner."""
    if hidden_local.ndim != 3 or hidden_local.shape[0] % tp_size:
        raise ValueError(
            f"expected [R,L,H] with R divisible by TP={tp_size}, "
            f"got {tuple(hidden_local.shape)}"
        )
    gathered = torch.empty(
        hidden_local.shape[0] // tp_size,
        hidden_local.shape[1] * tp_size,
        hidden_local.shape[2],
        dtype=hidden_local.dtype,
    )
    for group_index, group_base in enumerate(range(0, hidden_local.shape[0], tp_size)):
        group = hidden_local[group_base : group_base + tp_size].reshape(
            tp_size * hidden_local.shape[1], hidden_local.shape[2]
        )
        gathered[group_index].copy_(group)
    return gathered
    ...
    for rank in range(ranks):
        for token in range(positions.shape[1]):
            ...
            for layer in range(layers):
                physical = int(block_tables[rank, layer, 0, position // block_size])
...
_DRAFTER_DYNAMIC_CONTEXT_NAMES = frozenset(
    {"target_hidden", "context_position_ids", "context_slot_mapping"}
)
...
def _materialize_drafter_ios(stage, resident_names):
    ...
    context_positions = _materialize(decode_spec_map["context_position_ids"])
    context_slots = _materialize(decode_spec_map["context_slot_mapping"])
    decode_io["target_hidden"] = target_hidden[:, :DECODE_WIDTH].contiguous()
    decode_io["context_position_ids"] = context_positions[:, :DECODE_WIDTH].contiguous()
    decode_io["context_slot_mapping"] = context_slots[:, :, :DECODE_WIDTH].contiguous()
...
def _compiled_alias_compatible(
    ...
    *,
    ...
    if left.dtype != right.dtype or left.direction != right.direction:
        return False
    direction = getattr(left.direction, "name", str(left.direction).rsplit(".", 1)[-1])
...
def _copy_leader_request_table(target: torch.Tensor, source: torch.Tensor) -> None:
    if target.ndim != 3 or source.ndim != 2 or target.shape[0] != source.shape[0]:
        raise ValueError("cache tables must be [R,B,blocks] and [R,blocks] tensors")
    if source.shape[1] < target.shape[2]:
        raise ValueError("Prefill cache table is narrower than the Decode table")
    for leader in range(0, target.shape[0], TP_SIZE):
        target[leader, 0].copy_(source[leader, : target.shape[2]])
...
def _copy_group_request_table(target: torch.Tensor, source: torch.Tensor) -> None:
    ...
    if source.shape[1] < target.shape[2]:
        raise ValueError("Prefill cache table is narrower than the Decode table")
    target[:, 0].copy_(source[:, : target.shape[2]])
...
def _refresh_target_decode_metadata(stages, decode_io, prefill_tables, start_positions) -> None:
    ...
    for group_index, group_base in enumerate(range(0, decode_io["input_ids"].shape[0], TP_SIZE)):
        ...
        ori_table = prefill_tables["ori_block_table"][leader : leader + 1]
        hca_cmp_table = prefill_tables["hca_cmp_block_table"][leader : leader + 1]
        csa_cmp_table = prefill_tables["csa_cmp_block_table"][leader : leader + 1]
        idx_table = prefill_tables["idx_block_table"][leader : leader + 1]
...
def _prepare_drafter_prefill(stages, io, prefill_io, next_tokens) -> None:
    stage = stages["drafter"]
    gathered_hidden = _gather_prefill_target_hidden(prefill_io["dspark_target_hidden"])
    if gathered_hidden.shape != io["target_hidden"][::TP_SIZE].shape:
        raise ValueError(
            f"Prefill handoff shape {tuple(gathered_hidden.shape)} does not match drafter "
            f"owner shape {tuple(io['target_hidden'][::TP_SIZE].shape)}"
        )
    io["target_hidden"].zero_()
    io["target_hidden"][::TP_SIZE].copy_(gathered_hidden)
    ...
    io["anchor_positions"].zero_()
    io["anchor_positions"][:, 0] = gathered_hidden.shape[1] - 1
    positions = torch.arange(gathered_hidden.shape[1], dtype=torch.int32)
    io["context_position_ids"].copy_(
        positions.unsqueeze(0).expand(io["context_position_ids"].shape[0], -1)
    )
    valid = torch.ones_like(io["context_position_ids"], dtype=torch.bool)
    io["context_slot_mapping"].copy_(
        _context_slots(
            io["block_tables"],
            io["context_position_ids"],
            valid,
            stage.module.BLOCK_SIZE,
        )
    )
...
def _prepare_drafter_decode(stages, io, verified) -> None:
    ...
    io["context_position_ids"].copy_(verified.context_positions)
    io["context_slot_mapping"].copy_(
        _context_slots(
            io["block_tables"],
