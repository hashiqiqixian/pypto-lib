...
def _refresh_drafter_query_metadata(io, block_size: int) -> torch.Tensor:
...
def _refresh_drafter_rope_rows(io, drafter, utils, local_query_positions) -> None:
    """Refresh the bounded RoPE rows that match the current position metadata."""
    for prefix, positions in (
        ("context_group", io["context_group_position_ids"]),
        ("query", local_query_positions),
        ("query_group", io["query_group_position_ids"]),
    ):
        cos_name = f"{prefix}_freqs_cos"
        sin_name = f"{prefix}_freqs_sin"
        expected_shape = (*positions.shape, drafter.ROPE_DIM)
        if tuple(io[cos_name].shape) != expected_shape or tuple(
            io[sin_name].shape
        ) != expected_shape:
            raise ValueError(f"Drafter {prefix} RoPE rows do not match position metadata")
        rope_cos, rope_sin = utils.token_local_rope(
            drafter.M,
            0,
            positions,
            max_seq_len=drafter.MAX_SEQ_LEN,
            rope_dim=drafter.ROPE_DIM,
            dtype=io[cos_name].dtype,
        )
        io[cos_name].copy_(rope_cos.reshape(expected_shape))
        io[sin_name].copy_(rope_sin.reshape(expected_shape))
...
def _materialize_drafter_ios(stage, resident_names):
    ...
    context_freqs_cos = _materialize(decode_spec_map["context_group_freqs_cos"])
    context_freqs_sin = _materialize(decode_spec_map["context_group_freqs_sin"])
    ...
    decode_io["context_group_freqs_cos"] = context_freqs_cos[
        :, :group_decode_rows
    ].contiguous()
    decode_io["context_group_freqs_sin"] = context_freqs_sin[
        :, :group_decode_rows
    ].contiguous()
...
def _refresh_target_decode_metadata(stages, decode_io, prefill_tables, start_positions) -> None:
    ...
    for group_index, group_base in enumerate(range(0, decode_io["input_ids"].shape[0], TP_SIZE)):
        ...
        csa_state_slots = utils.state_slot_mapping(
            positions.to(torch.int64) % decode.csa.MAIN_STATE_STORAGE_LEN,
            csa_state_table,
            state_block_size=decode.CSA_MAIN_STATE_BLOCK_SIZE,
        ).reshape(-1)
        inner_state_slots = utils.state_slot_mapping(
            positions.to(torch.int64) % decode.csa.INNER_STATE_STORAGE_LEN,
            inner_state_table,
        ...
        compressed_rope_cos, compressed_rope_sin = utils.token_local_rope(
            decode.MODEL_CONFIG,
        ...
        decode_io["compressed_freqs_cos_local"][leader].copy_(compressed_rope_cos)
        decode_io["compressed_freqs_sin_local"][leader].copy_(compressed_rope_sin)
        ...
        for rank in range(group_base, group_base + TP_SIZE):
            ...
            decode_io["compressed_freqs_cos"][rank, :DECODE_WIDTH].copy_(
                compressed_rope_cos
            )
            decode_io["compressed_freqs_sin"][rank, :DECODE_WIDTH].copy_(
                compressed_rope_sin
            )
...
def _prepare_drafter_prefill(drafter, utils, io, prefill_io, next_tokens) -> None:
    ...
    local_query_positions = _refresh_drafter_query_metadata(io, drafter.BLOCK_SIZE)
    _refresh_drafter_rope_rows(io, drafter, utils, local_query_positions)
...
def _prepare_drafter_decode(stages, io, verified) -> None:
    ...
    local_query_positions = _refresh_drafter_query_metadata(
        io,
        drafter.module.BLOCK_SIZE,
    )
