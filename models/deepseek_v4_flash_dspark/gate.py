...
# can fan tokens across ranks. moe.py selects 16*EP fixture experts or retains
# the full configured expert set before importing this module.
...
def gate(
    ...
    with pl.at(level=pl.Level.CORE_GROUP, name_hint="gate_pre_route"):
        ...
        for zt in pl.range(active_tokens, T, T_TILE):
            ...
            inactive_x_norm_f16 = pl.tile.full([T_TILE, D], dtype=pl.FP16, value=0.0)
            ...
            for zs in pl.range(T_TILE):
                if zt + zs < T:
                    pl.write(x_norm_scale, [zt + zs, 0], pl.cast(0.0, pl.FP32))
            ...
            inactive_weights = pl.tile.full([T_TILE, TOPK_PAD], dtype=pl.FP32, value=0.0)
