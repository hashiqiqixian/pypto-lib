...
def draft_backbone(
    ...
    kv_caches_0: pl.InOut[
        pl.Tensor[[ORI_BLOCK_NUM, BLOCK_SIZE, 1, HEAD_DIM], pl.BF16]
    ],
    ...
    hc_attn_inv_rms_0: pl.Out[pl.Tensor[[T, 1], pl.FP32]],
    hc_attn_inv_rms_1: pl.Out[pl.Tensor[[T, 1], pl.FP32]],
    hc_attn_inv_rms_2: pl.Out[pl.Tensor[[T, 1], pl.FP32]],
    ...
    query_hidden = pl.create_tensor([T, D], dtype=pl.BF16)
    hidden_0_flat = pl.reshape(initial_hidden, [T, HC_MULT * D])
    main_x, query_token_ids, query_hidden, hidden_0_flat, lookup_tid = prepare_dspark_inputs(
        target_hidden,
        main_proj_weight,
        main_norm_weight,
        anchor_token_ids,
    ...
    query_slot_mapping_0 = pl.create_tensor([T_QUERY], dtype=pl.INT64)
    query_slot_mapping_1 = pl.create_tensor([T_QUERY], dtype=pl.INT64)
    query_slot_mapping_2 = pl.create_tensor([T_QUERY], dtype=pl.INT64)
    swa_indices_0 = pl.create_tensor([DSPARK_MAX_BATCH, DSPARK_SWA_INDEX_WIDTH], dtype=pl.INT32)
    swa_indices_1 = pl.create_tensor([DSPARK_MAX_BATCH, DSPARK_SWA_INDEX_WIDTH], dtype=pl.INT32)
    swa_indices_2 = pl.create_tensor([DSPARK_MAX_BATCH, DSPARK_SWA_INDEX_WIDTH], dtype=pl.INT32)
    swa_lens_0 = pl.create_tensor([DSPARK_MAX_BATCH], dtype=pl.INT32)
    swa_lens_1 = pl.create_tensor([DSPARK_MAX_BATCH], dtype=pl.INT32)
    swa_lens_2 = pl.create_tensor([DSPARK_MAX_BATCH], dtype=pl.INT32)
    with pl.at(
        level=pl.Level.CORE_GROUP,
        name_hint="dspark_layer_metadata_views",
        deps=[query_metadata_tid, visible_metadata_tid],
    ) as metadata_views_tid:
        for token in pl.range(T_QUERY):
            pl.write(query_slot_mapping_0, [token], pl.read(query_slot_mapping, [0, token]))
            pl.write(query_slot_mapping_1, [token], pl.read(query_slot_mapping, [1, token]))
            pl.write(query_slot_mapping_2, [token], pl.read(query_slot_mapping, [2, token]))
        for request in pl.range(DSPARK_MAX_BATCH):
            pl.write(swa_lens_0, [request], pl.read(swa_lens, [0, request]))
            pl.write(swa_lens_1, [request], pl.read(swa_lens, [1, request]))
            pl.write(swa_lens_2, [request], pl.read(swa_lens, [2, request]))
            for visible_offset in pl.range(DSPARK_SWA_INDEX_WIDTH):
                pl.write(swa_indices_0, [request, visible_offset], pl.read(swa_indices, [0, request, visible_offset]))
                pl.write(swa_indices_1, [request, visible_offset], pl.read(swa_indices, [1, request, visible_offset]))
                pl.write(swa_indices_2, [request, visible_offset], pl.read(swa_indices, [2, request, visible_offset]))
    first_layer_tid = lookup_tid
    hidden_1 = pl.create_tensor([T, HC_MULT, D], dtype=pl.FP32)
    dspark_draft_layer(
        hidden_0,
        hidden_0_flat,
        hc_attn_fn_0,
        ...
        routed_y_buf,
        combine_arrived,
        pl.const(40, pl.INT32),
        pl.cast(active_tokens, pl.INT32),
        my_rank,
        pl.const(1, pl.INT32),  # DSPARK_MOE_EPOCHS[0]; pl.const requires a literal.
        first_layer_tid,
    )
    rebase_1_tid = rebase_moe_signals(
        hidden_1,
        arrived,
        data_arrived,
        combine_arrived,
    ...
    barrier_1_tid = dspark_moe_barrier(
        moe_barrier_signal,
        my_rank,
        pl.const(1, pl.INT32),
        rebase_1_tid,
    ...
    hidden_2 = pl.create_tensor([T, HC_MULT, D], dtype=pl.FP32)
    hidden_1_flat = pl.reshape(hidden_1, [T, HC_DIM])
    dspark_draft_layer(
        hidden_1,
        hidden_1_flat,
        hc_attn_fn_1,
        ...
        routed_y_buf,
        combine_arrived,
        pl.const(41, pl.INT32),
        pl.cast(active_tokens, pl.INT32),
        my_rank,
        pl.const(2, pl.INT32),  # DSPARK_MOE_EPOCHS[1]; pl.const requires a literal.
        barrier_1_tid,
    )
    rebase_2_tid = rebase_moe_signals(
        hidden_2,
        arrived,
        data_arrived,
        combine_arrived,
    ...
    barrier_2_tid = dspark_moe_barrier(
        moe_barrier_signal,
        my_rank,
        pl.const(2, pl.INT32),
        rebase_2_tid,
    ...
    hidden_3 = pl.create_tensor([T, HC_MULT, D], dtype=pl.FP32)
    hidden_2_flat = pl.reshape(hidden_2, [T, HC_DIM])
    dspark_draft_layer(
        hidden_2,
        hidden_2_flat,
        hc_attn_fn_2,
        ...
        routed_y_buf,
        combine_arrived,
        pl.const(42, pl.INT32),
        pl.cast(active_tokens, pl.INT32),
        my_rank,
        pl.const(3, pl.INT32),  # DSPARK_MOE_EPOCHS[2]; pl.const requires a literal.
        barrier_2_tid,
    )
    ...
    head_hidden_flat = pl.reshape(
        head_hidden,
        [batch * DSPARK_QUERY_WIDTH, D],
    )
