...
# DSpark drafter constants
DSPARK_DRAFT_LAYERS = 3
DSPARK_TARGET_HIDDEN_LAYERS = (40, 41, 42)
...
DSPARK_ATTN_K_TILE = 64
DSPARK_SWA_INDEX_WIDTH = (
    (FLASH.sliding_window + DSPARK_QUERY_WIDTH + DSPARK_ATTN_K_TILE - 1)
    // DSPARK_ATTN_K_TILE
    * DSPARK_ATTN_K_TILE
)
DSPARK_MOE_EPOCHS = (1, 2, 3)

assert DSPARK_DRAFT_LAYERS == len(DSPARK_TARGET_HIDDEN_LAYERS)
assert DSPARK_QUERY_WIDTH < DSPARK_QUERY_PAD
assert DSPARK_MAX_BATCH == DECODE_BATCH // TP
assert DSPARK_MOE_TOKENS == MOE_TOKENS
assert DSPARK_NOISE_TOKEN_ID < FLASH.vocab_size
assert DSPARK_SWA_INDEX_WIDTH >= FLASH.sliding_window + DSPARK_QUERY_WIDTH
