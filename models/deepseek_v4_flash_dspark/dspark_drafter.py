...
def draft_layer(
    ...
    query_active = pl.slice(query_normed, [active_tokens, D], [0, 0])
    query_cos_active = pl.slice(query_freqs_cos, [active_tokens, ROPE_DIM], [0, 0])
    query_sin_active = pl.slice(query_freqs_sin, [active_tokens, ROPE_DIM], [0, 0])
    query_positions_active = pl.slice(query_positions, [active_tokens], [0])
    dspark_attention(
        query_active,
        query_group,
