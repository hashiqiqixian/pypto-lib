...
def draft_layer(
    ...
    query_cos_active = pl.slice(query_freqs_cos, [query_active_rows, ROPE_DIM], [0, 0])
    query_sin_active = pl.slice(query_freqs_sin, [query_active_rows, ROPE_DIM], [0, 0])
    query_positions_active = pl.slice(query_positions, [query_active_rows], [0])
    dspark_attention(
        query_active,
        query_group,
