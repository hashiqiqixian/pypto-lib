...
def _parse_dispatch_batch_argv():
    for index, token in enumerate(sys.argv):
        if token == "--dispatch-batch" and index + 1 < len(sys.argv):
            return int(sys.argv[index + 1])
        if token.startswith("--dispatch-batch="):
            return int(token.split("=", 1)[1])
    return 1


USER_BATCH = _parse_dispatch_batch_argv()
...
def prefill_layer_core(
    ...
    completion_anchor = pl.create_tensor([T, HC_MULT, D], dtype=pl.FP32)
    ...
    for request_id in pl.range(USER_BATCH):
        ...
        if valid_n > 0:
            ...
            x_next = pl.assemble(
                x_next, completion_anchor, [request_index, 0, 0, 0]
            )
