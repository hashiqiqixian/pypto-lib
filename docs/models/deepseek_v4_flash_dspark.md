...
- `draft_backbone` projects target hidden rows, precomputes the three isolated
  context-KV streams, runs three draft layers, and returns `head_hidden[B, 7, D]`;
- `markov_sample` applies the shared final norm and LM head, then performs seven
...
lookup_embedding(anchor/noise ids) -> BF16 query rows + FP32 HC rows
...
`dspark_draft_layer.py` is only the HC/MoE composition around that operator.
...
Each draft layer owns a distinct paged KV cache. The three MoE calls use epochs
1, 2, and 3, with signal rebasing and a cross-rank barrier between layers.

## Markov sampling

`markov_sample` computes all seven base-logit rows once. Each unrolled step
...
states and all three in/out KV caches, and mixes short-context, full-window,
page-boundary, and wrapped block-table cases in one batch.

```bash
python models/deepseek_v4_flash_dspark/dspark_backbone.py \
  --batch 4 --ep 2 -p a2a3 -d 0,1
python models/deepseek_v4_flash_dspark/dspark_backbone.py \
  --batch 16 --ep 2 -p a2a3 -d 0,1
python models/deepseek_v4_flash_dspark/markov_sample.py --batch 4 -p a2a3 -d 0
python models/deepseek_v4_flash_dspark/markov_sample.py --batch 16 -p a2a3 -d 0
```
