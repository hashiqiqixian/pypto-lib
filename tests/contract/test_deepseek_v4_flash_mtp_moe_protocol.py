...
def test_prefill_protocol_keeps_reduce_taskid_local() -> None:
    ...
    prefill_moe = _function(tree, "batched_prefill_moe")
...
def test_prefill_window_shapes_match_all_jit_and_host_boundaries() -> None:
    ...
    boundaries = {
        "moe.py": {
            "clear_batched_prefill_moe_signals": tuple(expected),
            "prefill_dispatch": tuple(expected),
            "prefill_combine": ("combine_arrived", "consumed"),
            "batched_prefill_moe": tuple(expected),
        },
    ...
    moe = _function(_tree("moe.py"), "batched_prefill_moe")
    parameter_names = [arg.arg for arg in moe.args.args]
    caller_trees = (_tree("prefill_fwd.py"), _tree("prefill_layer.py"))
    calls = [call for tree in caller_trees for call in _calls(tree, "batched_prefill_moe")]
    assert len(calls) == 6
    for call in calls:
...
def test_clear_batched_prefill_moe_signals_retires_then_resets_local_slots() -> None:
    clear = _function(_tree("moe.py"), "clear_batched_prefill_moe_signals")
    ...
    caller_trees = (_tree("prefill_fwd.py"), _tree("prefill_layer.py"))
    calls = [call for tree in caller_trees for call in _calls(tree, "clear_batched_prefill_moe_signals")]
    assert len(calls) == 2
    for call in calls:
