...
def test_fixed_capacity_parents_isolate_dynamic_attention_outputs_from_moe() -> None:
    ...
    output_pairs = {
        "prefill_fwd.py": (
            ("x_attn0_valid", "x_attn0_storage", "x_attn0", "token_count"),
            ("x_attn1_valid", "x_attn1_storage", "x_attn1", "token_count"),
            ("x_attn_csa_valid", "x_attn_csa_storage", "x_attn_csa", "token_count"),
            ("x_attn_hca_valid", "x_attn_hca_storage", "x_attn_hca", "token_count"),
            ("x_attn_last_valid", "x_attn_last_storage", "x_attn_last", "token_count"),
        ),
        "prefill_mtp.py": (("x_attn_valid", "x_attn_storage", "x_attn", "token_count"),),
        "prefill_layer.py": (("x_attn_valid", "x_attn_storage", "x_attn_tile", "valid_tok"),),
    }
    for filename, pairs in output_pairs.items():
        source = (MODEL / filename).read_text(encoding="utf-8")
        tree = ast.parse(source)
        for dynamic_name, storage_name, fixed_name, token_extent in pairs:
            ...
            assert f"{dynamic_name} = pl.slice({fixed_name}," not in source
            assert f"{fixed_name} = pl.assemble({fixed_name}, {dynamic_name}, [0, 0, 0])" in source
            ...
            for node in ast.walk(tree):
                ...
                if (
                    ...
                    and target.id == fixed_name
                    and isinstance(value, ast.Call)
                    and ast.unparse(value.func) == "pl.create_tensor"
                ):
                    fixed_creates.append(value)
            assert any(
                ast.unparse(call.args[0]) in {"[T, HC_MULT, D]", "[TOK_TILE, HC_MULT, D]"}
                for call in fixed_creates
            )
...
def test_hc_pre_materializes_comb_rows_as_vec_tiles_before_store() -> None:
    for name in ("_hc_pre_syncall", "_hc_pre_separate"):
        ...
        for row in range(4):
            ...
            assert f"row{row}_out = pl.load(comb_pad_store" in source
...
def test_dynamic_attention_tile_reductions_provide_scratch_tiles() -> None:
    ...
    assert "pl.pipeline(Q_ROPE_H_TILE" not in qkv_source
...
def test_qkv_rope_gathers_include_flattened_row_offsets() -> None:
    ...
    assert "qrp_swap_idx = pl.add(pl.cast(qrp_swap_f" in source
