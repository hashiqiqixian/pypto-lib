...
def test_prefill_drivers_size_active_runtime_rings() -> None:
    expected = {
        ("prefill_layer.py", "PREFILL_LAYER_RING_HEAP"): (
            "(0, 512 * 1024 * 1024, 2 * 1024 * 1024 * 1024, 1024 * 1024 * 1024)"
        ),
        ("prefill_fwd.py", "PREFILL_RING_HEAP"): (
            "(0, 512 * 1024 * 1024, 2 * 1024 * 1024 * 1024, 512 * 1024 * 1024)"
        ),
    }

    for (filename, name), value in expected.items():
        assignment = next(
            node
        ...
        assert ast.unparse(assignment.value) == value
...
def test_dynamic_indexer_q_projection_keeps_static_spmd_topology() -> None:
    source = ast.unparse(_function("prefill_indexer.py", "prefill_indexer"))

    assert "pl.spmd(qr_col_blocks, name_hint='prefill_idx_qr_proj')" in source
    assert "pl.range(0, work_tokens, QR_PROJ_ROW_TILE)" in source
    assert "qr_tasks" not in source
