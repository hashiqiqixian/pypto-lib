...
@pl.jit.host
def transport(
    indices: pl.Tensor[[ROUNDS,N_RANKS,T,TOPK],pl.INT32],
    x: pl.Tensor[[ROUNDS,N_RANKS,T,D],pl.INT8],
    scales: pl.Tensor[[ROUNDS,N_RANKS,1,T*K_SCALE],pl.UINT8],
    weights: pl.Tensor[[ROUNDS,N_RANKS,T,TOPK],pl.FP32],
    count: pl.Tensor[[ROUNDS,N_RANKS,1],pl.INT32],
    rx: pl.Out[pl.Tensor[[ROUNDS,N_RANKS,N_LOCAL,RECV_MAX,D],pl.INT8]],
    rw: pl.Out[pl.Tensor[[ROUNDS,N_RANKS,N_LOCAL,RECV_MAX],pl.FP32]],
    rr: pl.Out[pl.Tensor[[ROUNDS,N_RANKS,N_LOCAL,RECV_MAX],pl.INT32]],
    rc: pl.Out[pl.Tensor[[ROUNDS,N_RANKS,N_LOCAL,1],pl.INT32]],
    meta: pl.Out[pl.Tensor[[ROUNDS,N_RANKS,N_RANKS,N_LOCAL],pl.INT32]],
    output: pl.Out[pl.Tensor[[ROUNDS,N_RANKS,T,D],pl.BF16]],
    scale_out: pl.Out[pl.Tensor[[ROUNDS,N_RANKS,1,N_LOCAL*RECV_MAX*K_SCALE],pl.UINT8]],
):
    ...
    for step in pl.range(ROUNDS):
        for rank in pl.range(N_RANKS):
            ...
            rank_transport(indices[step,rank],x[step,rank],scales[step,rank],weights[step,rank],count[step,rank],rx[step,rank],rw[step,rank],rr[step,rank],rc[step,rank],meta[step,rank],output[step,rank],scale_out[step,rank],recv_meta,recv_x,recv_scale,recv_weights,recv_routes,arrived,data_arrived,routed_output,combine_arrived,rank,step+1,device=rank)
...
if __name__=='__main__':
    ...
    shapes={'rx':([ROUNDS,N_RANKS,N_LOCAL,RECV_MAX,D],torch.int8),'rw':([ROUNDS,N_RANKS,N_LOCAL,RECV_MAX],torch.float32),'rr':([ROUNDS,N_RANKS,N_LOCAL,RECV_MAX],torch.int32),'rc':([ROUNDS,N_RANKS,N_LOCAL,1],torch.int32),'meta':([ROUNDS,N_RANKS,N_RANKS,N_LOCAL],torch.int32),'output':([ROUNDS,N_RANKS,T,D],torch.bfloat16)}
    shapes['scale_out']=([ROUNDS,N_RANKS,1,N_LOCAL*RECV_MAX*K_SCALE],torch.uint8)
